...
- might need to remove old entries from keychain
    - keychain delete seems to fail silently unless you have password app open at the same time
...
- deleted the classic token to try again - failed
- deleted both and created a new classic token only 
    - updated in SourceTree by editing the account settings
    - works. did not have to delete anything from keychain
